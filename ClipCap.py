...
from decoder.patch_mapping_network import Upsampling
from decoder.deconv_utils import Dict2Class
from decoder.deconv_config import config as dc_config
...
class MappingNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.pure_config = self.load_pure_config("/data/twkim/smallcap/data/qformer_config.json")
        self.purecap = Upsampling(self.pure_config)

    def load_pure_config(self, args):
        import json
        default_config = dc_config
        q_config = json.load(open(args))
        qformer_config = dict(q_config,**default_config)
        qformer_config = Dict2Class(qformer_config)
        qformer_config.cross_attention_reduce_factor = 4
        qformer_config.num_hidden_layers = 1
        qformer_config.patch_num = 2 ####

        qformer_config.num_attention_heads = 12
        qformer_config.hidden_size = 768
        qformer_config.encoder_hidden_size = qformer_config.hidden_size

        qformer_config.sequence_length = 50
        qformer_config.k = qformer_config.num_query_tokens
        qformer_config.noise_var = 0.016
        return qformer_config

    def forward(self, x: torch.Tensor, ret_feat: torch.Tensor) -> torch.Tensor:
        """
        Args:
...
class ClipCaptionModel(nn.Module):
    ...
    def forward(
        ...
        ret_feat: Optional[torch.Tensor] = None,
    ) -> Tuple[torch.Tensor, ...]:
        """
        Args:
