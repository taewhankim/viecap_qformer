...
class CaptionsDataset(Dataset):
    ...
    def __init__(
        ...
        if debug: # debug
            captions_with_entities = captions_with_entities
        ...
        ## 수정
        ret_path = "/data/twkim/viecap/feature/ret_add_noise_image_caps_train2.json"
        ret_caps = json.load(open(ret_path))
        self.ret_caps_list = []

        cap_feat = {c.rstrip():f for d,c,f in captions_with_entities}
        ...
        for caption_with_entities in captions_with_entities:
            if self.using_clip_features:
                ...
                ## 수정
                self.ret_caps_list.append(list(map(cap_feat.get, ret_caps[temp_caption.rstrip()])))
            else:
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, ...]:
        ...
        """
        ## 수정
        ret_feat = self.ret_caps_list[item]
        if any(elem is None for elem in ret_feat):
            raise Exception("None error")
        else:
            ret_feat = torch.stack(ret_feat)
        ...
        return  self.args, captions_clip, captions_gpt_tokens, masks, discrete_tokens, ret_feat


def collate(batch):
    batch_size = len(batch)
    args = batch[0][0]
    _, captions_clip, captions_gpt_tokens, masks, discrete_tokens, ret_feat = zip(*batch)
    ...
    return captions_clip, captions_gpt_tokens, captions_tokens_for_loss, masks, hard_prompts_length, ret_feat