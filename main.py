...
def train(
    ...
    for epoch in range(epochs):
        ...
        train_loss_sum = 0
        # training
        for idx, (captions_clip, captions_gpt_tokens, captions_tokens_for_loss, masks, hard_prompts_length, ret_feat) in enumerate(dataloader):
            model.zero_grad()
            if not args.using_clip_features:
                with torch.no_grad():
            ...
            with torch.cuda.amp.autocast(enabled = args.use_amp):                
                if args.using_hard_prompt:
                    outputs = model(continuous_prefix, captions_gpt_tokens, hard_prompts_length, masks, ret_feat.to(device))
...
def main():
    ...
    parser.add_argument('--language_model', default = 'gpt2-large', help = 'gpt2, facebook/opt-350m')
    ...
    parser.add_argument('--path_of_datasets', default = '/data/twkim/viecap/ann/coco/coco_with_entities.pickle')
    parser.add_argument('--out_dir', default = '/data/twkim/viecap/checkpoints', help = 'the path of output')
    ...
    parser.add_argument('--path_of_objects_vocabs', default = '/data/twkim/viecap/ann/vocabulary/all_objects_attributes_relationships.pickle')
