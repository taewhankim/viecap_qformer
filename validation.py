...
def validation_coco_flickr30k(
    ...
    eval_df = json.load(open('/data/twkim/viecap/feature/ret_add_noise_image_caps_test2.json','r'))
    test_ret_id = [k for k,v in eval_df.items()]
    test_ret_caps = [i for k,v in eval_df.items() for i in v]
    bs = 1000
    ret_feats = []
    for idx in tqdm(range(0, len(test_ret_caps), bs)):
        caps = test_ret_caps[idx:idx + bs]
        with torch.no_grad():
            ret_feat_batch = text_model.encode_text(clip.tokenize(caps).to(device))
            ret_feats.append(ret_feat_batch)

    ret_feats = torch.concat(ret_feats)
    eval_df_dict = {}
    for id, idx in zip(test_ret_id, range(0,len(ret_feats),5)):
        eval_df_dict[id] = ret_feats[idx:idx+5]
    ...
    for idx, item in tqdm(enumerate(annotations)):
        ...
        ret_feat = eval_df_dict[image_id]

        image_features /= image_features.norm(2, dim = -1, keepdim = True)
        continuous_embeddings = model.mapping_network(image_features,ret_feat.unsqueeze(0)).view(-1, args.continuous_prompt_length, model.gpt_hidden_size)
        if args.using_hard_prompt:
            logits = image_text_simiarlity(texts_embeddings, temperature = args.temperature, images_features = image_features)
            detected_objects, _ = top_k_categories(entities_text, logits, args.top_k, args.threshold) # List[List[]], [[category1, category2, ...], [], ...]
            detected_objects = detected_objects[0] # infering single image -> List[category1, category2, ...]
    ...
    out_json_path = os.path.join(args.out_path, f'{args.name_of_datasets}_generated_captions_{str(args.epoch_num)}.json')
...
def main(args) -> None:
    ...
    if args.name_of_entities_text == 'visual_genome_entities':
        ...
        entities_text = load_entities_text(args.name_of_entities_text, '/data/twkim/viecap/ann/vocabulary/coco_categories.json', not args.disable_all_entities)
        if args.prompt_ensemble:
            texts_embeddings = clip_texts_embeddings(entities_text, f'/data/twkim/viecap/ann/vocabulary/coco_embeddings_{clip_name}_with_ensemble.pickle')
        else:
            texts_embeddings = clip_texts_embeddings(entities_text, f'/data/twkim/viecap/ann/vocabulary/coco_embeddings_{clip_name}.pickle')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--language_model', default = 'gpt2-large')
    ...
    parser.add_argument('--weight_path', default = '/data/twkim/viecap/checkpoints/train_coco/coco_prefix-0014.pt')
    parser.add_argument('--image_folder', default = '/data/twkim/viecap/ann/coco/val2014/')
    ...
    parser.add_argument('--epoch_num', type = int, default = 14, help = 'epoch num')
